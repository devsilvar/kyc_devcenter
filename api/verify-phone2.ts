...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const signatureString = `${timestamp}${PARTNER_ID}sid_request`; // confirm `sid_request`
    ...
    const smileIDRequestBody = {
      ...
      match_fields: { first_name: firstName, last_name: lastName },
      partner_params: { job_id: `job-${Date.now()}`, user_id: `user-${Date.now()}` },
    };
